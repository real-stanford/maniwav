...
def main(input, output, out_res, out_fov, compression_level, 
    ...
    # dump audio
    block_size = 800
    def audio_to_zarr(audio_array_l, audio_array_r, mp4_path, tasks):
        audio_path = mp4_path[:mp4_path.rfind('/')] + '/audio.wav'
        tasks = sorted(tasks, key=lambda x: x['frame_start'])
        curr_task_idx = 0
        
        for frame_idx, block in enumerate(sf.blocks(audio_path, blocksize=block_size)):
            block = np.pad(block, ((0, block_size - block.shape[0]), (0, 0)), 'constant', constant_values=(0, 0))
            ...
            if frame_idx < tasks[curr_task_idx]['frame_start']:
                ...
                audio_array_l[buffer_idx] = block[:, 0]
                audio_array_r[buffer_idx] = block[:, 1]
                buffer_idx += 1
    ...
    if 'mic_0' in out_replay_buffer.data:  # left channel
        audio_array_l = out_replay_buffer.data['mic_0']
        n,b = audio_array_l.shape
        assert b == 800
        new_n = out_replay_buffer['robot0_eef_pos'].shape[0]
        audio_array_l.resize((new_n,b))
    else:
        audio_array_l = out_replay_buffer.data.require_dataset(
            name='mic_0',
            shape=(out_replay_buffer['robot0_eef_pos'].shape[0], block_size),
            chunks=(1, block_size),
            dtype=np.float64
        )
    
    if 'mic_1' in out_replay_buffer.data: # right channel
        audio_array_r = out_replay_buffer.data['mic_1']
        n,b = audio_array_r.shape
        assert b == 800
        new_n = out_replay_buffer['robot0_eef_pos'].shape[0]
        audio_array_r.resize((new_n,b))
    else:
        audio_array_r = out_replay_buffer.data.require_dataset(
            name='mic_1',
            shape=(out_replay_buffer['robot0_eef_pos'].shape[0], block_size),
            chunks=(1, block_size),
            dtype=np.float64
    ...
    with tqdm(total=len(vid_args)) as pbar:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
            ...
            for mp4_path, tasks in vid_args:
                ...
                futures.add(executor.submit(audio_to_zarr, 
                    audio_array_l, audio_array_r, mp4_path, tasks))
